...
class MessageHandler:
    ...
    def _check_captcha(self, message: Message, cursor, db) -> bool:
        ...
        # Captcha Handler - 用户正在回答验证码
        if (captcha := self.cache.get(f"captcha_{message.from_user.id}")) is not None:
            success, error_msg = self.captcha_manager.verify_captcha(message.from_user.id, message.text, db)
            if not success:
        ...
        if not self.captcha_manager.is_user_verified(message.from_user.id, db):
            ...
            captcha_type = self.cache.get("setting_captcha")
            question, error_msg = self.captcha_manager.generate_captcha(message.from_user.id, captcha_type, db)
            ...
            match captcha_type:
                ...
                case _:
                    ...
                    self.bot.send_message(self.group_id,
                                          _("Invalid captcha setting") + f": {captcha_type}")
                    return False
        return True
