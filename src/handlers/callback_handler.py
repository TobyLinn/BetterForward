...
class CallbackHandler:
    ...
    def _handle_verify_button(self, call: types.CallbackQuery, data: dict):
        ...
        if not token:
            self.bot.answer_callback_query(call.id, _("Invalid verification token"), show_alert=True)
            return
        
        # 验证按钮验证码
        import sqlite3
        with sqlite3.connect(self.db_path) as db:
            success, error_msg = self.captcha_manager.verify_button_captcha(user_id, token, db)
            
            if success:
                self.captcha_manager.set_user_verified(user_id, db)
                self.bot.answer_callback_query(call.id, _("Verification successful!"), show_alert=True)
                ...
                self.bot.send_message(user_id, error_msg)
